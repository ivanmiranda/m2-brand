 ...
 * @license    
 */
namespace Pengo\Brand\Model\ResourceModel;
...
class Brand extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    protected function _construct(){
    	$this->_init('Pengo_Brand','brand_id');
    }
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['Pengo_Brand_store' => $this->getTable('Pengo_Brand_store')],
                $this->getMainTable() . '.brand_id = Pengo_Brand_store.brand_id',
                []
                )->where(
                'status = ?',
                1
                )->where(
                'Pengo_Brand_store.store_id IN (?)',
                $storeIds
                )->order(
                'Pengo_Brand_store.store_id DESC'
                )->limit(
                1
                );
            }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('Pengo_Brand_store')],
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('Pengo_Brand_store'), $condition);

        $condition = ['brand_id = ?' => (int)$object->getId()];
        $this->getConnection()->delete($this->getTable('Pengo_Brand_product'), $condition);
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $table = $this->getTable('Pengo_Brand_store');
        ...
        if(null !== ($object->getData('products'))){
            $table = $this->getTable('Pengo_Brand_product');
    ...
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if ($id = $object->getId()) {
                ...
                ->from($this->getTable('Pengo_Brand_product'))
                ->where(
                    'brand_id = '.(int)$id
                    );
    ...
    public function lookupStoreIds($brandId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('Pengo_Brand_store'),
            'store_id'
            )
        ->where(
            'brand_id = ?',
